...
grass <- st_read("/Users/sarahkmaebius/Documents/Fall_2020_Reed/Thesis/tree_canopy/grass_polygon.shp")
...
```{r}
# save results
#write.csv(poly_join_reprojected,'poly_join_reprojected1.csv')
poly_join_reprojected <- read.csv('poly_join_reprojected1.csv')
poly_join_reprojected <- poly_join_reprojected %>%
  mutate(X = factor(X))
#write.csv(val_combined,'val_combined1.csv')
val_combined <- read.csv('val_combined1.csv')

# tidy pixel table
pixels_data <- sp::merge(val_combined, poly_join_reprojected, by.x = "ID", by.y = "X")

# with ndvi index (1 if ndvi >= 2.5, 0 otherwise)
pixels_data <- pixels_data %>%
  select(-c("X.x", "X.y", "St_Wdth")) %>%
