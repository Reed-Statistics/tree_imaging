...
rf_model_grouped <- readRDS("data/rf_model_grouped.rds")
```
...
predRaster <- function(stack) {
  ...
  results_RF_group <- raster::predict(stack, rf_model_grouped, progress = "text")
  results <- stack(results_RF, results_RF_group)
  stopCluster(cl)
  return(results)
}
...
r_list_RF_group <- list(results_a[[2]] == 7, results_b[[2]] == 7, results_c[[2]] == 7,
                  results_d[[2]] == 7, results_e[[2]] == 7)
m_RF <- do.call(merge, r_list_RF)
m_RF_group <- do.call(merge, r_list_RF_group)

# compare rasters
r_matches <- m_RF == m_RF_group
freq(r_matches)
freq(r_matches)[2, ]/(freq(r_matches)[2, ] + freq(r_matches)[1, ])
# (r_matches == 1)/((r_matches == 0) + (r_matches == 1)) percentage in common: 83%
plot(r_matches == 1, main = "RF C = 7 and RF C = 5 Predictions Comparisons",
...
plot(m_RF_group, main = "RF Grouped Western Redcedar Predictions", 
     labels = F, xaxt = 'n', yaxt = 'n', legend = F, col = "darkgreen")
plot(portland_wgs84, add = T)
```
...
p1 <- plot(m_RF, main = "RF C = 7 with pdxTrees", 
     labels = F, xaxt = 'n', yaxt = 'n', legend = F, col = "darkgreen")
plot(dat_transformed, col = rgb(red = 0.2, green = 0.2, blue = 1.0, alpha = 0.2), cex = .2, add = T)
plot(portland_wgs84, add = T)

plot(m_RF_group, main = "RF C = 5 with pdxTrees", 
     labels = F, xaxt = 'n', yaxt = 'n', legend = F, col = "darkgreen")
plot(dat_transformed, col = rgb(red = 0.2, green = 0.2, blue = 1.0, alpha = 0.2), cex = .2, add = T)
plot(portland_wgs84, add = T)
...
street_mask_group <- mask(m_RF_group, dat_transformed)
freq(street_mask)[1, 2]/nrow(dat_transformed) # testing accuracy estimates for Western redcedars
# 52%
freq(street_mask_group)[1, 2]/nrow(dat_transformed) # testing accuracy estimates for Western redcedars
# 66%
...
# Reed plot
plot(reed_stack, main = "", labels = F, xaxt = 'n', yaxt = 'n', legend = F, col = "darkgreen")
