...
results_rf <- rbind(cbind(rf_random1$resample, Model = rep("5", nrow(rf_random1$resample))), 
                 cbind(rf_random2$resample, Model = rep("7", nrow(rf_random2$resample))), 
                 cbind(rf_random3$resample, Model = rep("8", nrow(rf_random3$resample))),
                 cbind(rf_random$resample, Model = rep("8 ntry = 2", nrow(rf_random3$resample))))
```
...
# collect resamples
results_svm <- rbind(cbind(svm1$resample, Model = rep("svmLinear", nrow(svm1$resample))), 
...
results_r <- results_rf %>%
  mutate(Model = recode(as.character(Model), '5'= '5 predictors',
                        ...
                        '8 ntry = 2' = '8 predictors with mtry = 2')) %>%
  group_by(Model) %>%
