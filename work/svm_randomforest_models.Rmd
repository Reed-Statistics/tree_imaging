...
rf2_grouped <- train(Cmmn_Nm~., data = train2_grouped, method = "rf", trControl = control)
rf3_grouped <- train(Cmmn_Nm~., data = train3_grouped, method = "rf", trControl = control)
```
...
# fix class names
# ******************************
# need to fix make.names
# run all svm models again and rf svm model comparison
# save confusion matrices, results tables, hyperparameter plots, and comparison plots
# train <- train %>%
#   mutate(Cmmn_Nm = make.names(Cmmn_Nm))
# train_grouped <- train_grouped %>%
#   dplyr::select(c(red, green, blue, ir, ndvi, red_green, red_blue, blue_green, Cmmn_Nm)) %>%
#   mutate(Cmmn_Nm = make.names(Cmmn_Nm))
...
svm7 <- train(Cmmn_Nm ~., data = train_grouped, method = "svmRadial", 
...
# save best models
# saveRDS(rf3_grouped, "data/rf_model.rds")
# saveRDS(svm2, "data/svm_model.rds")
```
...
test_results_grouped$RF <- predict(rf3_grouped, test_grouped)
