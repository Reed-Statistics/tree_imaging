...
## Training Models
For both the random forest model and the support vector machines model, the tidy pixels data is arbitrarily divided in half into a training dataset and a testing dataset and the selected variables for training the models are the four bands (red, green, blue, infrared) and an NDVI variable. The `randomForest` function from the `randomForest` package [@random_forest] is used to train a random forest model on the training dataset to predict the tree species. The function takes parameters for data, the predictive variable, and the subset data that should be used to train the model. By default, the function grows 500 trees and samples cases with replacement.

The `svm` function from the `e1071` package [@svm] is used to train a support vector machine model on the training dataset to classify pixels into types of trees. By default, data are scaled to zero mean and unit variance and the kernel is radial.
...
A second mask is applied to the raster image that contains only the vegetation pixels to distinguish tree pixels from grass pixels. Inspired by [@qian_new_2020], the...
