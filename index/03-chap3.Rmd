...
where $\hat{p}_{mk}$ is the proportion of observations in the $m^{\text{th}}$ region from the $k^{\text{th}}$ class [@james_introduction_2013]. 

```{r decisiontree, results="asis", echo=FALSE, fig.cap="A single decision tree (left) displays splits at different predictors. Random forests (right) are constructed by aggregating many decision trees constructed from bootstrapped samples of the data and from splits made from a random subset of predictors.", out.width='70%', fig.align='center'}
include_graphics("figure/decision_tree.png")
```
...
(1) Choose $\beta_0, \beta_1, ..., \beta_p$ to maximize margin of hyperplane (M)

(2) Apply constraints to ensure that observations are M away from the hyperplane and on the correct side:
...
(1) Choose $\beta_0, \beta_1, ..., \beta_p, \epsilon_1, ..., \epsilon_n$ to maximize margin of hyperplane (M)
...
```{r diagrams, results="asis", echo=FALSE, fig.cap="Left: The linear kernel creates a linear boundary between two classes. Middle: The radial kernel successful models data where one class is surrounded by the other class. Right: The polynomial kernel is slightly more flexible than the linear kernel.", out.width='70%', fig.align='center'}
include_graphics("figure/hyperplane.png")
```
...
The `caret` package [@caret] in RStudio provides the framework for the modelling performed in this work. It stands for classification and regression training, and it contains functions for facilitating the process of creating training models in R and determining the tuning parameters. 

### Random Forest Tuning

The `train` function from the `caret` package [@caret] with the specified `method = rf` option is used to train a random forest model on the training dataset to predict the tree species. The function takes parameters for data, the predictive variable, and the method of training the model. To perform 10-fold cross validation, the `trainControl` function is used to create an object that tells the `train` function to perform cross validation 10 times with a different fold left out each time. The function automatically tries multiple number of tries at each split (mtry) and determines the optimal number through cross-validation. Models were considered with total predictors around 7 or 8 predicting classes of size 7 (individual species) or 5 (oaks and maples grouped together):
...
```{r resultsTestRF, echo=F, warning=F, message=F, fig.cap='Chart', out.width='100%'}
library(caret)
test_results <- read.csv('~/tree_imaging/data/test_results.csv', check.names = F)

results_tab_RF <- confusionMatrix(test_results$RF, test_results$Class)


knitr::kable(results_tab_RF$byClass, "latex", caption = "Overall accuracy of 7 predictor RF (5 class) model on test data", digits = 2) %>%
  kable_styling(latex_options = c("scale_down", "hold_position")) %>%
  column_spec(2:12, width = "0.6in")
```

```{r resultsTestSVM, echo=F, warning=F, message=F, fig.cap='Chart', out.width='100%'}
results_tab_SVM <- confusionMatrix(test_results$SVM, test_results$Class)

knitr::kable(results_tab_SVM$byClass, "latex", caption = "Overall accuracy of Radial SVM (5 class) model on test data", digits = 2) %>%
  kable_styling(latex_options = c("scale_down", "hold_position")) %>%
  column_spec(2:12, width = "0.6in")
```


<!-- # ```{r resultsTest2RF, echo=F, warning=F, message=F, fig.cap='Chart', out.width='100%'} -->
<!-- # test_results_redceder <- test_results %>% -->
<!-- #   dplyr::filter(Class %in% "Western Redcedar") -->
<!-- #  -->
<!-- # results_redcedar_tabRF <- confusionMatrix(test_results_redceder$RF, test_results_redceder$Class) -->
<!-- #  -->
<!-- # knitr::kable(results_redcedar_tabRF$byClass, "latex", caption = "Western redcedar accuracy of 7 predictor RF (5 class) model on test data", digits = 2) %>% -->
<!-- #   kable_styling(latex_options = c("scale_down", "hold_position")) %>% -->
<!-- #   column_spec(2:12, width = "0.6in") -->
<!-- # ``` -->
<!-- #  -->
<!-- # ```{r resultsTest2SVM, echo=F, warning=F, message=F, fig.cap='Chart', out.width='100%'} -->
<!-- # test_results_redceder <- test_results %>% -->
<!-- #   dplyr::filter(Class %in% "Western Redcedar") -->
<!-- #  -->
<!-- # results_redcedar_tabSVM <- confusionMatrix(test_results_redceder$SVM, test_results_redceder$Class) -->
<!-- #  -->
<!-- # knitr::kable(results_redcedar_tabSVM$byClass, "latex", caption = "Western redcedar accuracy of Radial SVM (5 class) model on test data", digits = 2) %>% -->
<!-- #   kable_styling(latex_options = c("scale_down", "hold_position")) %>% -->
<!-- #   column_spec(2:12, width = "0.6in") -->
<!-- # ``` -->
...
knitr::kable(poly_results, "latex", caption = "Overall accuracy of radial SVM model with 5 classes and 7 predictor RF model with 5 classes for polygons in test data. A correct prediction is considered to be a polygon with more than half of the pixels correctly classified.")
```

```{r polyTabRed, echo=F, warning=F, message=F, fig.cap='Chart', out.width='100%'}
poly_test_rf_redcedar <- read.csv('~/tree_imaging/data/poly_test_rf_redcedar.csv', check.names = F)
poly_test_svm_redcedar <- read.csv('~/tree_imaging/data/poly_test_svm_redcedar.csv', check.names = F)
...
poly_results_redcedar <- cbind(Type = c("SVM C = 5", "SVM C = 5", "RF P = 7, C = 5", "RF P = 7, C = 5"), 
                      rbind(poly_test_svm_redcedar, poly_test_rf_redcedar)) %>%
  rename("Polygons" = n)

knitr::kable(poly_results_redcedar, "latex", caption = "Western Redcedar accuracy of radial SVM model with 5 classes and 7 predictor RF model with 5 classes for polygons in test data. A correct prediction is considered to be a polygon with more than half of the pixels correctly classified.")
```
