...
```{r rfmtry, results="asis", echo=FALSE, fig.cap="Comparison of number of randomly selected predictors for each split from random forest model with 8 predictors included", out.width='40%', fig.align='center'}
...
```{r resultsR, echo=F, warning=F, message=F, fig.cap='Chart', out.width='50%'}
results_r <- read.csv('~/tree_imaging/results_r.csv', check.names = F)
results_s <- read.csv('~/tree_imaging/results_s.csv', check.names = F)

# knitr::kable(results_r, "latex", caption = "Accuracy of random forest models with different number of predictors and model with best ntry parameter.")
